...
final class modulesController extends cpLoader
{
    ...
    public function clean()
    {
        ...
        Q("DELETE FROM `#__mdd_fields` WHERE `module_id` NOT IN(?li)", array(array_keys($modules)));
...
class ". $f_name ."Module extends Module
{
    ...
    public function listMethod()
    {
        ...
        # Получение списка
        #
        if (!\$this->mcache_enable || (\$this->caching == 1 && !(\$". $f_name ." = \$this->getCache('_module_". $f_name ."'))))
        {
            ...
            \$this->setCache('_module_". $f_name ."', \$". $f_name .");
        }
    ...
    {
        if (!\$this->mcache_enable || (\$this->caching == 1 && !(\$". $f_name ." = \$this->getCache('_module_". $f_name ."_item'))))
        {
            \$". $f_name ." = Q(\"SELECT * FROM `#_mdd_". $f_name ."` WHERE `visible`='1' AND `id`=?i\", array( \$item ))->row();
            \$". $f_name ."['date'] = Dates(\$". $f_name ."['date'], \$this->locale);

            \$this->setCache('_module_". $f_name ."_item', \$". $f_name .");
        }
        ...
        \$breadcrumbs = array(
                ...
                'link'      => '/' . \$this->module_root . '/' . \$". $f_name ."['id'] . '-' . \$". $f_name ."['system']
        );
