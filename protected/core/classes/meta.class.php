...
class Meta extends MDD
{
	...
	public function __construct($module_id)
	{
		...
		if (isset($_COOKIE['modulePager_' . $this->module_info['id']]))
		{
			$this->pager = $_COOKIE['modulePager_' . $this->module_info['id']];
		}
	...
	function filterModule($module_id = 0, $meta_module = array())
	{
		$filter = array();
	...
	function sortModule($module_id = 0, $meta_module = array())
	{
		return array();
	...
	public function inAsocArray($needle = array(), $haystack = array())
	{
	...
	public function listModule($fields = array())
	{
		$arr = array();
		$res = array();
		...
		{
			$cookie_sort = unserialize(from_base($_COOKIE['moduleSort'])) ;
		...
		$pager['all_items']	= Q("SELECT COUNT(`id`) AS `count` FROM `".$this->table_main."` LIMIT 1")->row('count');
	...
	function itemExtend($meta_item = array())
	{
	...
	function getListInfo($arr = array())
	{
		foreach ($arr as $k => $item)
		{
			if (isset($item['f_type']))
			{
				...
				if (in_array($item['f_type'], array('file', 'image')))
				{
					...
					$arr[$k]['list'] = $f->getFilesByGroup($arr[$k]['value'], 'original');
					$arr[$k]['json_list'] = json_encode($arr[$k]['list']);
				}
	...
	//	
	function tree_list_tree($a_list = array(), $id=0, $all_tree)
	{
		if (!isset($a_list[$id]) || empty($a_list[$id])) return array();
