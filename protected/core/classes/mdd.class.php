...
class MDD
{
	...
	private $necessary_fields = array(
		"id" 				=> array('f_name' => "ID", 'f_sys_name' => "id", 'f_width' => 0, 'f_type' => "int", "ord" => "0", "in_list" => "0"),
		"ord" 				=> array('f_name' => "Сортировка", 'f_sys_name' => "ord",'f_width' => 0,  'f_type' => "int", "ord" => "1000", "in_list" => "1"),
		"visible" 			=> array('f_name' => "Отображать",	'f_sys_name' => "visible", 'f_width' => 0,  'f_type' => "tinyint", "ord" => "1010", "in_list" => "1"),
		"created" 			=> array('f_name' => "Дата создания", 'f_sys_name' => "created",'f_width' => 0,  'f_type' => "int", "ord" => "1020",	"in_list" => "0"),
		"updated" 			=> array('f_name' => "Дата изменения", 'f_sys_name' => "updated",'f_width' => 0,  'f_type' => "int", "ord" => "1030", "in_list" => "0"),
		"uid" 				=> array('f_name' => "Пользователь", 'f_sys_name' => "uid",'f_width' => 0,  'f_type' => "int", 'f_settings' => array('f_table_name' => '#__usr_users', 'f_table_field' => 'name'), "ord" => "1040", "in_list" => "0"),
		"gid" 				=> array('f_name' => "Группа", 'f_sys_name' => "gid", 'f_width' => 0,  'f_type' => "int", 'f_settings' => array('f_table_name' => '#__usr_groups', 'f_table_field' => 'name'), "ord" => "1050", "in_list" => "0"),
	);
	...
	public function getGroupModules($modules = array(), $groups = array())
	{
		if (!empty($groups))
		{
			if (!empty($modules))
	...
	public function check_recursive($arr)
	{
		...
		// exit( Qb("SELECT `id`, `f_sys_name`, `f_settings` FROM `".$this->table_fields."` WHERE `f_settings`!='' AND `module_id`=?i", array( $arr['id'] )) );
	...
	{
		$fields = Q("SELECT `id`, `module_id`, `f_name`, `f_sys_name`, `f_type`, `f_width`, `f_settings`, `f_index`, `f_unique`, `ord`, `in_list` FROM `".$this->table_fields."` WHERE `module_id`=?i GROUP BY `f_sys_name` ORDER BY `ord`", array( $module_id ))->all();
		...
		if (!empty($fields))
		{
			...
			{
				if ($all_fields or !isset($this->necessary_fields[ $ar['f_sys_name'] ]))
				{
	...
	public function updateModule($id = 0, $fields = array())
	{
	...
	protected function insertSystemFields($id, $fields = array())
	{
	...
	{
		$table_name = str_replace('#', DB_PREF, $table_name);

		// $a = Q("SHOW TABLES LIKE ?s;", array($table_name))->row();
		$count = Q("SELECT COUNT(*) as `count` FROM `information_schema`.`tables` WHERE `table_schema` LIKE ?s AND `table_name` LIKE ?s LIMIT 1", array( DB_BASE, $table_name ))->row('count');
