...
class Files
{
	...
	public function deleteFile($id = 0)
	{
		...
		$files = Q("SELECT `id`, `fid`, `file` FROM `".$this->table."` WHERE `id`=?i OR `fid`=?i", array( $id, $id ))->all();
	...
	public function getFilesByGroupPager($gid = array(), $page=0,$l)
	{
		$photos = array();
		$count = 0;
		$start = $page * $l;
	...
	protected function getPrepared($sample = array(), $columns = array(), $count = 0)
	{
		$result = array();
	...
	public function getFilesByGroupCount($gid = '', $prefix = array(), $columns = array(), $count = 1, $rand = false)
	{
	...
	public function getFilesByGroup($gid = '', $prefix = array(), $columns = array(), $group = false)
	{
		if (is_array($prefix))
		{
			$where = "`prefix` IN (?ls)";
